...
(ert-deftest suderman/escape-is-bound-to-the-shared-dispatcher ()
  (should (eq (lookup-key meow-insert-state-keymap (kbd "<escape>"))
              #'meow-insert-exit))
  (should (eq (lookup-key meow-normal-state-keymap (kbd "<escape>"))
              #'suderman/meow-escape))
  (should (eq (lookup-key meow-motion-state-keymap (kbd "<escape>"))
              #'suderman/meow-escape))
  (should (eq (lookup-key global-map (kbd "<escape>"))
              #'suderman/meow-escape)))
...
(ert-deftest suderman/leader-files-and-toggles-use-focused-commands ()
  ...
  (dolist (binding '(("c" . display-fill-column-indicator-mode)
                     ...
                     ("h" . hl-line-mode)
                     ("n" . suderman/toggle-line-numbers)
                     ("r" . read-only-mode)
                     ("s" . jinx-mode)
                     ("v" . visual-line-mode)
                     ("w" . whitespace-mode)))
    (should (eq (lookup-key suderman/leader-toggle-map (kbd (car binding)))
                (cdr binding))))
  (should-not (lookup-key suderman/leader-toggle-map (kbd "l"))))
