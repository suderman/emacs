      ...
      (unwind-protect
          (progn
            ...
            (should (equal (mapcar (lambda (binding) (nth 2 binding))
                                   (cdr tool-bar-map))
                           (list "Ctrl" "Meta" (string #xF018F)
                                 (string #xF097B) (string #xF0256)
                                 "Tab" "Esc")))
...
(ert-deftest suderman/custom-toolbar-images-use-native-pbm-source ()
...
(ert-deftest suderman/custom-toolbar-native-labels-show-modifier-state ()
  (let ((original-tool-bar-map (default-value 'tool-bar-map))
        (map (make-sparse-keymap)))
    (define-key map [control] '(menu-item "Ctrl" ignore))
    (define-key map [meta] '(menu-item "Meta" ignore))
    (unwind-protect
        (progn
          (set-default 'tool-bar-map map)
          (cl-letf (((symbol-function 'tool-bar--flush-cache) #'ignore)
                    ((symbol-function 'force-mode-line-update) #'ignore))
            (let ((modifier-bar-modifier-list '(control)))
              (suderman/tool-bar-refresh)
              (should (equal (nth 2 (assq 'control (cdr map))) "CTRL"))
              (should (equal (nth 2 (assq 'meta (cdr map))) "Meta")))
            (let ((modifier-bar-modifier-list '(meta)))
              (suderman/tool-bar-refresh)
              (should (equal (nth 2 (assq 'control (cdr map))) "Ctrl"))
              (should (equal (nth 2 (assq 'meta (cdr map))) "META")))))
      (set-default 'tool-bar-map original-tool-bar-map))))
