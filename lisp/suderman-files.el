...
(defconst suderman/dirvish-help-keys
  '(("h" . "Parent directory")
    ...
    ("M-H" . "Shrink window width")
    ("M-J" . "Increase window height")
    ("M-K" . "Decrease window height")
...
(use-package dirvish
  ...
  (dolist (map (list dired-mode-map dirvish-mode-map))
    ...
    (keymap-set map "M-H" #'suderman/shrink-window-width)
    (keymap-set map "M-J" #'suderman/enlarge-window-height)
    (keymap-set map "M-K" #'suderman/shrink-window-height)
    (keymap-set map "M-L" #'suderman/enlarge-window-width)
